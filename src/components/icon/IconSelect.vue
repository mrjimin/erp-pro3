<template>
    <div class="icon-selector w100 h100">
        <el-popover placement="bottom" :width="fontIconWidth" trigger="click" transition="el-zoom-in-top"
            ...
            <template #reference>
                <el-input ref="inputWidthRef" v-model="fontIconSearch" :placeholder="fontIconPlaceholder"
            ...
            <template #default>
                <div class="icon-selector-warp">
                    <div class="icon-selector-warp-title flex">
                        <div class="flex-auto input-x">
                            <el-input v-model="searchText" :placeholder="title"></el-input>
                        </div>
                        <div v-if="type === 'all'" class="icon-selector-warp-title-tab">
                            <span :class="{ 'span-active': fontIconType === 'ali' }" class="ml10" title="iconfont 图标"
                                @click="onIconChange('ali')">ali</span>
                            <span :class="{ 'span-active': fontIconType === 'ele' }" class="ml10" title="elementPlus 图标"
                                @click="onIconChange('ele')">ele</span>
                            <span :class="{ 'span-active': fontIconType === 'awe' }" class="ml10"
...
import { initIconfont, initIcon } from '@/utils/getStyleSheets';
...
const initFontIconData = async (type: string) => {
    ...
    if (state.searchText) {
        state.fontIconSheetsList = iconClass.filter((item) => item.indexOf(state.searchText) >= 0)
    } else {
