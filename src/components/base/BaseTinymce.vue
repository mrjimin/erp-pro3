<template>
    <div class="tinymce-box">
        <Editor v-model="value" :tinymce-script-src="localFile" :api-key="tiny.apiKey" :init="tiny.init"
            cloud-channel="5" />
    </div>
</template>
...
const localFile = ref('/tinymce/tinymce.min.js');
...
const tiny = reactive({
    ...
    init: {
        ...
        // 文件上传处理函数
        file_picker_callback: function (callback: (arg0: string, arg1: { title: string; }) => void, value: any, meta: { filetype: string; }) {
            ...
            inputElem.onchange = async () => {
                ...
                let file = inputElem.files?.[0] as File; //获取文件信息
                let src = await uploadFile(file)
